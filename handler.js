  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        if (!isNumber(user.otp)) user.otp = -1
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (plugin.verif == true && _user.verif == false) { // Butuh daftar?
          fail('unverif', m, this)
          continue
        }
        m.isCommand = true
        ...
        if (!isPrems && plugin.limitbuy && global.db.data.users[m.sender].limitbuy < plugin.limitbuy * 1) {
          this.reply(m.chat, `Kamu Sudah Mencapai Limit Membeli Barang, Silahkan Tunggu 10 Menit Lagi`, m)
          continue // Limit habis
        }
...
global.dfail = (type, m, conn) => {
  ...
  let msg = {
    ...
    unverif: 'Saya Mencium Bau Robot Nih... Yuk Verifikasi Gmail Dulu Biar Bisa Akses Fiturku .regmail <Gmail mu>\nContoh :\n.regmail support@miniwon.online\n.regmail miniwon@gmail.com',
